...
def upgrade() -> None:
    with op.batch_alter_table('filter_record') as batch_op:
        batch_op.add_column(sa.Column('ferment_id', sa.Unicode(length=64),
                      sa.ForeignKey('ferment_record.ferment_id', name='fk_filter_record_ferment_id_ferment_record'), nullable=True))
        batch_op.create_index('ix_filter_record_ferment_id', ['ferment_id'])


def downgrade() -> None:
    prep_drop_columns(op.get_bind(), 'filter_record', ['ferment_id'])
    with op.batch_alter_table('filter_record') as batch_op:
        batch_op.drop_index('ix_filter_record_ferment_id')
        batch_op.drop_column('ferment_id')
