...
def upgrade() -> None:
    ...
    with op.batch_alter_table('filter_order_tank') as batch_op:
        batch_op.add_column(sa.Column('filter_id', sa.Unicode(64), sa.ForeignKey('filter_record.filter_id', name='fk_filter_order_tank_filter_id_filter_record'), nullable=True))
        batch_op.create_index('ix_filter_order_tank_filter_id', ['filter_id'])
