...
def upgrade() -> None:
    op.create_table(
        ...
        sa.UniqueConstraint('batch_code', name='uq_brew_batch_batch_code'),
    )
    ...
    with op.batch_alter_table('brew_material_usage') as batch_op:
        ...
        batch_op.add_column(sa.Column('batch_id', sa.Unicode(length=64), sa.ForeignKey('brew_batch.batch_id', name='fk_brew_material_usage_batch_id_brew_batch'), nullable=False))
...
def downgrade() -> None:
    ...
    with op.batch_alter_table('brew_material_usage') as batch_op:
        ...
        batch_op.add_column(sa.Column('brew_id', sa.Unicode(length=64), sa.ForeignKey('brew_record.brew_id', name='fk_brew_material_usage_brew_id_brew_record'), nullable=False))
        batch_op.create_index('ix_brew_material_usage_brew_id', ['brew_id'])
