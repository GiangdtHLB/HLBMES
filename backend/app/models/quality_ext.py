...
from sqlalchemy import Text, Date, DateTime, Float, String
...
class QCParameter(Base):
    ...
    param_id: Mapped[str] = mapped_column(String(64), primary_key=True, default=new_id)
    ...
    stage: Mapped[Optional[str]] = mapped_column(String(255), nullable=True)  # nau|len_men|loc|chiet
...
class CAPA(Base):
    ...
    capa_id: Mapped[str] = mapped_column(String(64), primary_key=True, default=new_id)
    capa_code: Mapped[str] = mapped_column(String(64), unique=True, index=True)
    deviation_id: Mapped[Optional[str]] = mapped_column(String(64), nullable=True, index=True)
    title: Mapped[str] = mapped_column(String(255))
    capa_type: Mapped[str] = mapped_column(String(255), default="corrective")  # corrective | preventive
    severity: Mapped[str] = mapped_column(String(255), default="minor")
    # open → investigation → action → verification → closed
    state: Mapped[str] = mapped_column(String(255), default="open")
    root_cause: Mapped[Optional[str]] = mapped_column(String(255), nullable=True)
    action_plan: Mapped[Optional[str]] = mapped_column(String(255), nullable=True)
    effectiveness: Mapped[Optional[str]] = mapped_column(String(255), nullable=True)
...
class Sample(Base):
    ...
    sample_id: Mapped[str] = mapped_column(String(64), primary_key=True, default=new_id)
    sample_code: Mapped[str] = mapped_column(String(64), unique=True, index=True)
    scope_type: Mapped[str] = mapped_column(String(255), default="batch")
    ...
    status: Mapped[str] = mapped_column(String(255), default="registered")
    test_set: Mapped[Optional[str]] = mapped_column(String(255), nullable=True)  # csv tên parameter cần test
