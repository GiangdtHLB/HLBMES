...
class StockMovement(Base):
    ...
    movement_id: Mapped[str] = mapped_column(String(64), primary_key=True, default=new_id)
    # receipt=nhập | issue=xuất | return=nhập hoàn | transfer=xuất sang ngang | adjust
    movement_type: Mapped[str] = mapped_column(String(255), index=True)
    ...
    mode: Mapped[Optional[str]] = mapped_column(String(255), nullable=True)  # de_nghi | tu_do | sang_ngang
    reason: Mapped[Optional[str]] = mapped_column(Text, nullable=True)
    ref_doc: Mapped[Optional[str]] = mapped_column(String(255), nullable=True)
