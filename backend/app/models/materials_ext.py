...
class Dispense(Base):
    ...
    dispense_id: Mapped[str] = mapped_column(String, primary_key=True, default=new_id)
    dispense_code: Mapped[str] = mapped_column(String, unique=True, index=True)
    batch_id: Mapped[str] = mapped_column(ForeignKey("batch_execution.batch_id"), index=True)
    mode: Mapped[str] = mapped_column(String, default="dispense")   # dispense | backflush
    status: Mapped[str] = mapped_column(String, default="issued")   # issued (đã trừ tồn)
