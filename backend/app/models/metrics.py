...
class ProcessReading(Base):
    ...
    reading_id: Mapped[str] = mapped_column(String, primary_key=True, default=new_id)
    batch_id: Mapped[str] = mapped_column(ForeignKey("batch_execution.batch_id"), index=True)
    parameter: Mapped[str] = mapped_column(String, index=True)  # temperature | gravity | pH ...
    value: Mapped[float] = mapped_column(Float)
    unit: Mapped[Optional[str]] = mapped_column(String, nullable=True)
    # source/ingest timestamp (tài liệu §8.3) — ở MVP gộp làm một mốc UTC.
    ts: Mapped[datetime] = mapped_column(DateTime(timezone=True), default=utcnow, index=True)
    quality: Mapped[str] = mapped_column(String, default="good")  # good | stale | bad


class OEERecord(Base):
    __tablename__ = "oee_record"

    oee_id: Mapped[str] = mapped_column(String, primary_key=True, default=new_id)
    line: Mapped[str] = mapped_column(String, index=True)
    shift: Mapped[str] = mapped_column(String, default="A")
