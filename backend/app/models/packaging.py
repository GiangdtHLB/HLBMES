...
class PackagingType(Base):
    ...
    pkg_id: Mapped[str] = mapped_column(Unicode(64), primary_key=True, default=new_id)
    code: Mapped[str] = mapped_column(Unicode(64), unique=True, index=True)
    name: Mapped[str] = mapped_column(Unicode(255))
    category: Mapped[str] = mapped_column(Unicode(255), index=True)   # vo_chai | ket_gong | keg
    material: Mapped[Optional[str]] = mapped_column(Unicode(255), nullable=True)  # glass/plastic/steel...
...
class PackagingMove(Base):
    ...
    move_id: Mapped[str] = mapped_column(Unicode(64), primary_key=True, default=new_id)
    pkg_id: Mapped[str] = mapped_column(ForeignKey("packaging_type.pkg_id"), index=True)
    kind: Mapped[str] = mapped_column(Unicode(255))   # nhap | xuat | thu_hoi | loai_bo | kiem_ke
