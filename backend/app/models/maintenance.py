...
class Equipment(Base):
    ...
    eq_type: Mapped[Optional[str]] = mapped_column(Unicode(255), nullable=True)   # loại thiết bị
    system: Mapped[Optional[str]] = mapped_column(Unicode(255), nullable=True)    # hệ thống
    location: Mapped[Optional[str]] = mapped_column(Unicode(255), nullable=True)
    status: Mapped[str] = mapped_column(Unicode(255), default="running")  # running/idle/maintenance/broken


class SparePart(Base):
    __tablename__ = "spare_part"

    part_id: Mapped[str] = mapped_column(Unicode(64), primary_key=True, default=new_id)
    code: Mapped[str] = mapped_column(Unicode(64), unique=True, index=True)
    name: Mapped[str] = mapped_column(Unicode(255))
    uom: Mapped[str] = mapped_column(Unicode(255), default="cái")
...
class Incident(Base):
    ...
    incident_id: Mapped[str] = mapped_column(Unicode(64), primary_key=True, default=new_id)
    incident_code: Mapped[str] = mapped_column(Unicode(64), unique=True, index=True)
    ...
    status: Mapped[str] = mapped_column(Unicode(255), default="open")     # open/in_progress/resolved/closed
...
class MaintenancePlan(Base):
    ...
    plan_type: Mapped[str] = mapped_column(Unicode(255), default="bao_tri")  # bao_tri/kiem_tra/tu_bo
...
class Calibration(Base):
    ...
    calib_id: Mapped[str] = mapped_column(Unicode(64), primary_key=True, default=new_id)
    equipment_id: Mapped[Optional[str]] = mapped_column(ForeignKey("equipment.equipment_id"), index=True)
    name: Mapped[str] = mapped_column(Unicode(255))
    # phong_xa | van_an_toan | hieu_chuan_tbd | yc_nnvat
    calib_type: Mapped[str] = mapped_column(Unicode(255), default="hieu_chuan_tbd")
    ...
    result: Mapped[Optional[str]] = mapped_column(Unicode(255), nullable=True)  # pass/fail
    status: Mapped[str] = mapped_column(Unicode(255), default="valid")          # valid/due/overdue
