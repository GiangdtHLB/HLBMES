...
from sqlalchemy import Text, Date, DateTime, Float, ForeignKey, String, UniqueConstraint
...
class EnergyGroup(Base):
    ...
    group_id: Mapped[str] = mapped_column(String(64), primary_key=True, default=new_id)
    code: Mapped[str] = mapped_column(String(64), unique=True, index=True)
    name: Mapped[str] = mapped_column(String(255))
    unit: Mapped[str] = mapped_column(String(255), default="kWh")


class EnergyArea(Base):
    __tablename__ = "energy_area"

    area_id: Mapped[str] = mapped_column(String(64), primary_key=True, default=new_id)
    code: Mapped[str] = mapped_column(String(64), unique=True, index=True)
    name: Mapped[str] = mapped_column(String(255))
