...
class ChemicalUsage(Base):
    ...
    stage: Mapped[str] = mapped_column(Unicode(255), default="nau")  # nau/len_men/loc/chiet/cip
    chemical: Mapped[str] = mapped_column(Unicode(255))
...
class YeastLot(Base):
    ...
    yeast_lot_id: Mapped[str] = mapped_column(Unicode(64), primary_key=True, default=new_id)
    code: Mapped[str] = mapped_column(Unicode(64), unique=True, index=True)
    strain: Mapped[str] = mapped_column(Unicode(255), default="W-34/70")
    generation: Mapped[int] = mapped_column(Integer, default=1)
    source_tank: Mapped[Optional[str]] = mapped_column(Unicode(255), nullable=True)
    source_batch_id: Mapped[Optional[str]] = mapped_column(Unicode(64), nullable=True)
...
class YeastIssue(Base):
    ...
    issue_id: Mapped[str] = mapped_column(Unicode(64), primary_key=True, default=new_id)
