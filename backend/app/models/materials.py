...
class MaterialLot(Base):
    ...
    lot_id: Mapped[str] = mapped_column(String, primary_key=True, default=new_id)
    lot_code: Mapped[str] = mapped_column(String, unique=True, index=True)
    ...
    lot_type: Mapped[str] = mapped_column(String, default="material")  # material | brew | package ...

    supplier_lot: Mapped[Optional[str]] = mapped_column(String, nullable=True)
    quantity: Mapped[float] = mapped_column(Float, default=0.0)
    uom: Mapped[str] = mapped_column(String, default="kg")
    status: Mapped[str] = mapped_column(String, default=LotStatus.AVAILABLE.value)
...
class GenealogyEdge(Base):
    ...
    relation: Mapped[str] = mapped_column(String, default=GenealogyRelation.CONSUME.value)
    quantity: Mapped[Optional[float]] = mapped_column(Float, nullable=True)
    uom: Mapped[Optional[str]] = mapped_column(String, nullable=True)
    source_event: Mapped[Optional[str]] = mapped_column(String, nullable=True)
