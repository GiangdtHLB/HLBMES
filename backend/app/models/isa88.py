...
class BatchPhaseRun(Base):
    ...
    unit_class: Mapped[Optional[str]] = mapped_column(Unicode(255), nullable=True)  # brewhouse|fv|filter|filler|cip|sip
    up_name: Mapped[str] = mapped_column(Unicode(255))        # unit procedure
    op_name: Mapped[str] = mapped_column(Unicode(255))        # operation
    phase_name: Mapped[str] = mapped_column(Unicode(255))     # phase
