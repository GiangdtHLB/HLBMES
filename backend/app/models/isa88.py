...
from sqlalchemy import Text, JSON, DateTime, ForeignKey, Integer, String
...
class BatchPhaseRun(Base):
    ...
    unit_class: Mapped[Optional[str]] = mapped_column(String(255), nullable=True)  # brewhouse|fv|filter|filler|cip|sip
    up_name: Mapped[str] = mapped_column(String(255))        # unit procedure
    op_name: Mapped[str] = mapped_column(String(255))        # operation
    phase_name: Mapped[str] = mapped_column(String(255))     # phase
    state: Mapped[str] = mapped_column(String(255), default=PhaseState.RUNNING.value, index=True)
