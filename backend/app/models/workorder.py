...
class WorkOrder(Base):
    ...
    wo_id: Mapped[str] = mapped_column(String, primary_key=True, default=new_id)
    wo_code: Mapped[str] = mapped_column(String, unique=True, index=True)
    ...
    line: Mapped[Optional[str]] = mapped_column(String, index=True)   # dây chuyền/khu
    shift: Mapped[Optional[str]] = mapped_column(String, default="A")  # ca A/B/C
    scheduled_date: Mapped[datetime] = mapped_column(Date, index=True)
    priority: Mapped[int] = mapped_column(Integer, default=5)
    status: Mapped[str] = mapped_column(String, default=WorkOrderState.PLANNED.value, index=True)
