...
def _a_formula(client, headers, product_id, material_code, qty):
    ...
    assert r.status_code == 201, r.text
    formula = r.json()
    client.post(f"/api/formulas/{formula['formula_id']}/activate", headers=headers)
    return formula


def _create_order(client, admin_h, product_id, formula_id, batch_count=2, overrides=None):
    r = client.post("/api/brewing/orders", headers=admin_h, json={
        "order_code": f"LN-QS-{new_id()[:8]}", "product_id": product_id, "formula_id": formula_id,
...
def test_preview_suggests_split_using_workshop_stock_first(client, admin_h):
    ...
    formula = _a_formula(client, admin_h, product_id, mat_code, qty=10)  # 10kg/mẻ
...
def test_preview_all_from_workshop_when_stock_covers_full_demand(client, admin_h):
    ...
    product_id = _a_product(client, admin_h)
    formula = _a_formula(client, admin_h, product_id, mat_code, qty=10)
...
def test_create_order_persists_suggested_split_when_no_override(client, admin_h):
    ...
    order_id = _create_order(client, admin_h, product_id, formula["formula_id"], batch_count=2)
...
def test_update_order_applies_new_override(client, admin_h):
    ...
    r = client.put(f"/api/brewing/orders/{order_id}", headers=admin_h, json={
        "order_code": f"LN-QS-UPD-{new_id()[:6]}", "product_id": product_id, "formula_id": formula["formula_id"],
