...
Từ 2026-09-18: fifo_ok cho đường /consume trực tiếp không còn MÃI MÃI None — được SUY LUẬN LẠI
(fifo_computed=True) bằng cách dựng lại tồn theo lô tại Kho phân xưởng đúng lúc batch.start_at,
xem services/dispense.py::batch_dispense_summary."""
...
def test_summary_shows_lot_code_for_direct_consume(client, admin_h):
    ...
    # Không đi qua dispense() -> không có xác nhận FIFO THẬT lúc cấp liệu, nhưng đây là lô DUY
    # NHẤT của vật tư này nên suy luận lại vẫn kết luận được ĐÚNG FIFO (fifo_computed=True).
    assert row["fifo_ok"] is True
    assert row["fifo_computed"] is True


def test_summary_infers_fifo_false_for_direct_consume_of_newer_lot(client, admin_h):
    """Cùng đường /consume trực tiếp nhưng lần này CỐ Ý tiêu thụ lô MỚI HƠN trong khi lô CŨ HƠN
    vẫn còn tồn ở Kho phân xưởng tại đúng lúc mẻ bắt đầu — suy luận lại phải ra fifo_ok=False
    (không phải None), đối lập với test ở trên (lô duy nhất -> True)."""
    _clear_seed_batch_9002(client, admin_h)

    mat = client.post("/api/materials", headers=admin_h,
                      json={"code": "SUM-CONS02", "name": "Vật tư summary consume 2", "uom": "kg"})
    assert mat.status_code == 201, mat.text
    material_id, code = mat.json()["material_id"], mat.json()["code"]

    older = client.post("/api/warehouse/receive", headers=admin_h, json={
        "material_id": material_id, "quantity": 20, "uom": "kg", "location": "Kho phân xưởng"})
    assert older.status_code == 200, older.text
    older_id = older.json()["lot_id"]
    newer = client.post("/api/warehouse/receive", headers=admin_h, json={
        "material_id": material_id, "quantity": 20, "uom": "kg", "location": "Kho phân xưởng"})
    assert newer.status_code == 200, newer.text
    newer_id = newer.json()["lot_id"]
    newer_code = next(l["lot_code"] for l in client.get("/api/lots", headers=admin_h).json()
                      if l["lot_id"] == newer_id)

    bt = client.post("/api/beer-types", headers=admin_h,
                     json={"code": "BT-SUMCONS2", "name": "Loại SUM consume 2"})
    assert bt.status_code == 201, bt.text
    recipe = client.post("/api/recipes", headers=admin_h,
                         json={"code": "CT-SUMCONS2", "name": "Recipe SUM consume 2",
                              "beer_type_id": bt.json()["beer_type_id"]})
    assert recipe.status_code == 201, recipe.text
    prod = client.post("/api/products", headers=admin_h,
                       json={"code": "PRD-SUMCONS2", "name": "Dich SUM consume 2", "uom": "L",
                            "beer_type_id": bt.json()["beer_type_id"]})
    assert prod.status_code == 201, prod.text
    v = client.post(f"/api/recipes/{recipe.json()['recipe_id']}/versions", headers=admin_h,
                    json={"base_qty": 100, "base_uom": "L", "product_id": prod.json()["product_id"],
    ...
    # Cố ý tiêu thụ lô MỚI (newer_id), bỏ qua lô cũ hơn (older_id) vẫn còn nguyên 20kg.
    r = client.post(f"/api/batches/{batch_id}/consume", headers=admin_h,
                    json={"lot_id": newer_id, "quantity": 5})
    assert r.status_code == 200, r.text
    ...
    row = next(l for l in summary.json() if l["material_code"] == code)
    assert row["actual"] == 5.0
    assert row["lot_codes"] == [newer_code]
    assert row["fifo_ok"] is False
    assert row["fifo_computed"] is True
