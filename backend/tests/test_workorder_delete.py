...
def _a_wo(client, admin_h, production_order_id, line="Nấu A", recipe_version_id=None):
...
def test_delete_wo_before_dispatch(client, admin_h, lager_beer_type_id, lager_recipe_version_id):
    order_id = _a_production_order(client, admin_h, "PO-WODEL-001", lager_beer_type_id)
    # Lệnh SX (ERP) chỉ chọn Loại bia lúc lập (product_id None) — WO cần Dịch bia cụ thể nên
    # phải truyền recipe_version_id lúc lập (xem services/workorders.py::create_wo).
    wo_id = _a_wo(client, admin_h, order_id, recipe_version_id=lager_recipe_version_id)
...
def test_delete_wo_blocked_after_dispatch(client, admin_h, lager_beer_type_id, lager_recipe_version_id):
    order_id = _a_production_order(client, admin_h, "PO-WODEL-002", lager_beer_type_id)
    wo_id = _a_wo(client, admin_h, order_id, recipe_version_id=lager_recipe_version_id)
...
def test_delete_wo_requires_manage_perm(client, admin_h, lager_beer_type_id, lager_recipe_version_id):
    vanhanh_h = _login(client, "vanhanh", "123456")
    order_id = _a_production_order(client, admin_h, "PO-WODEL-003", lager_beer_type_id)
    wo_id = _a_wo(client, admin_h, order_id, recipe_version_id=lager_recipe_version_id)
