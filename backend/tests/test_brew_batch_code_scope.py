...
def test_same_batch_code_blocked_across_different_brews_same_year(client, admin_h):
    ...
    a2 = client.post(f"/api/brewing/brews/{brew_a}/batches", headers=admin_h,
                     json={"batch_code": "101", "started_at": "2026-01-15T08:00:00"})
    assert a2.status_code == 201, a2.text

    b2 = client.post(f"/api/brewing/brews/{brew_b}/batches", headers=admin_h,
                     json={"batch_code": "101", "started_at": "2026-03-01T08:00:00"})
    assert b2.status_code == 409, b2.text


def test_batch_code_still_unique_within_same_brew(client, admin_h):
    brew = _a_brew(client, admin_h, "BATCHSCOPE-C")
    first = client.post(f"/api/brewing/brews/{brew}/batches", headers=admin_h,
                        json={"batch_code": "102", "started_at": "2026-01-20T08:00:00"})
    assert first.status_code == 201, first.text

    dup = client.post(f"/api/brewing/brews/{brew}/batches", headers=admin_h,
                      json={"batch_code": "102", "started_at": "2026-01-21T08:00:00"})
    assert dup.status_code == 409, dup.text


def test_batch_code_freed_after_delete_even_for_different_brew(client, admin_h):
    ...
    created = client.post(f"/api/brewing/brews/{brew_old}/batches", headers=admin_h,
                          json={"batch_code": "103", "started_at": "2026-02-01T08:00:00"})
    ...
    blocked = client.post(f"/api/brewing/brews/{brew_other}/batches", headers=admin_h,
                         json={"batch_code": "103", "started_at": "2026-02-02T08:00:00"})
...
def test_batch_code_resets_each_year(client, admin_h):
    ...
    b2026 = client.post(f"/api/brewing/brews/{brew_2026}/batches", headers=admin_h,
                       json={"batch_code": "104", "started_at": "2026-06-01T08:00:00"})
    ...
    b2027 = client.post(f"/api/brewing/brews/{brew_2027}/batches", headers=admin_h,
                       json={"batch_code": "104", "started_at": "2027-01-05T08:00:00"})
    assert b2027.status_code == 201, b2027.text


def test_qc_result_not_shared_between_batches_with_same_code(client, admin_h):
    ...
    ba = client.post(f"/api/brewing/brews/{brew_a}/batches", headers=admin_h,
                     json={"batch_code": "105", "started_at": "2028-05-01T08:00:00"})
    ...
    bb = client.post(f"/api/brewing/brews/{brew_b}/batches", headers=admin_h,
                     json={"batch_code": "105", "started_at": "2029-05-01T08:00:00"})
