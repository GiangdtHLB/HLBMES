...
- **SSE streaming chat** (`POST /api/ai/chat/stream`): token hiện dần như gõ máy + sự kiện gọi tool, vẫn lưu ConversationMemory. Backend dùng Claude `messages.stream` (vòng lặp tool-use) hoặc stream engine luật từng từ khi offline; chạy generator trong threadpool nên **không nghẽn event loop**. Frontend tiêu thụ bằng `fetch` + `ReadableStream` (gửi được header Authorization, parse SSE `data:`). Rate-limit + hạn mức ngày áp cho cả `/chat` lẫn `/chat/stream`.
- **/metrics (Prometheus)** (`metrics_prom.py`, không thêm dep): đếm `mes_http_requests_total{route,status}` + thời gian, `mes_ai_calls/errors/tokens_total`, `mes_ratelimit_blocks_total`, gauge `mes_audit_chain_intact`/`mes_audit_entries` (tính khi scrape). Mỗi request có `X-Request-ID`; **alert** dựa trên WARN log (rate-limit/LLM lỗi) + metric.
- **Worker tác vụ nền** (`models/jobs.py`, `services/jobs.py`): `ThreadPoolExecutor` + registry handler; `POST /api/jobs {kind}` → chạy nền, `GET /api/jobs[/{id}]` poll trạng thái/kết quả (cô lập theo user). Handler mẫu: `ai_report` (báo cáo vận hành AI), `recall` (truy xuất/recall lô). UI tab Trợ lý AI có nút **📋 Báo cáo nền**. Quy mô lớn: thay bằng Celery/RQ — interface `submit()/get()` giữ nguyên.
- **Rate-limit pluggable**: backend in-process (mặc định) hoặc **Redis** (đặt `MES_REDIS_URL`, fixed-window + daily INCR; tự fallback in-process nếu Redis lỗi). Interface `check_rate_limit()` không đổi.
- **Kiểm thử restore backup** `scripts/test_restore.sh`: dump → nạp DB scratch → verify (đếm bản ghi + audit hash-chain) → xoá scratch (không đụng prod). Lập lịch cron hằng tuần (mẫu trong file).
- **Module hoá frontend**: tách thư viện biểu đồ `CH` sang `frontend/charts.js` (nạp trước `app.js`); thứ tự `charts.js → app.js → views_ext.js`.

> **Migration:** chuỗi Alembic `4b0bfd0900bd → 89f74fef30e3 → a1b2c3d4e5f6` (bảng `job`). `alembic upgrade head` cho prod; dev SQLite dùng `create_all`.
