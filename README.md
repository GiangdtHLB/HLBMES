...
> **P2 còn lại (khi quy mô/đồng thời tăng):** worker/queue cho tác vụ AI dài; **monitoring** `/metrics` + cảnh báo lỗi LLM/timeout + kiểm thử restore backup định kỳ; **module hoá `frontend/app.js`** sâu hơn. Rate-limit hiện in-process — nhiều worker/replica nên chuyển sang Redis (token bucket), interface `check_rate_limit()` giữ nguyên.
